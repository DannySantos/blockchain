...
danny_coin.create_transaction(Transaction.new("pams_address", "barrys_address", 450))
danny_coin.create_transaction(Transaction.new("barrys_address", "pams_address", 50))

danny_coin.mine_pending_transactions("dannys_address")

puts "Pam's balance (expect -400): #{danny_coin.check_wallet_balance("pams_address")}"
puts "Barry's balance (expect 400): #{danny_coin.check_wallet_balance("barrys_address")}"
puts "Danny's balance (expect 0): #{danny_coin.check_wallet_balance("dannys_address")}"

danny_coin.mine_pending_transactions("dannys_address")
puts "Danny's balance (expect 100): #{danny_coin.check_wallet_balance("dannys_address")}"