...
dc.add_transaction(Transaction.new("dees_address", "macs_address", 1000))
dc.add_transaction(Transaction.new("macs_address", "dees_address", 500))

dc.mine_pending_transactions("charlies_address")

puts "Dee's balance (expect -500): #{dc.check_wallet_balance("dees_address")}"
puts "Mac's balance (expect 500): #{dc.check_wallet_balance("macs_address")}"
puts "Charlie's balance (expect 0): #{dc.check_wallet_balance("charlies_address")}"

dc.mine_pending_transactions("charlies_address")

puts "Charlie's balance (expect 100): #{dc.check_wallet_balance("charlies_address")}"
